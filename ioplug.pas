...
//      by Silhwan Hyun
//
//  Added 5 functions for Winamp General Purpose Plug-in, at 2008-09-08
...
//  Modifed record TWinampGPP
//   - added function "init2" for TBassPlayer at 2008-07-06
//   - added functions : TitleBarHighlighted, SetTitleBar at 2008-08-09
...
//---------------------- For VisDrawer.dll & General Purpose Plug-in ------------------------
const
  GPPHDR_VER = $10;
...
type
 ...
 //  ex. OrgWindowProc := Pointer(SetWindowLong(WindowHandle, GWL_WNDPROC, LongInt(@NewWindowProc));
  ...
  PVisDrawerMod = ^TVisDrawerMod;  // Modified version of Winamp GPP for Winamp-like visualization
  TVisDrawerMod = record
    version      : Integer;
    description  : PAnsiChar;

    Init         : function: Integer; cdecl;

  // Init2, TitleBarHighlighted and SetTitleBar are added functions for TBassPlayer.
    Init2        : function(var InitWinPos : TInitWinPos): Integer; cdecl;
    TitleBarHighlighted : function: Boolean; cdecl;
    SetTitleBar : procedure(HighLight : Boolean); cdecl;
...
var
 ...
 // getVisHeader : function : pointer; stdcall;
  ...
  getVisDrawerHeader : function : pointer; stdcall;

function InitInputDLL(DLL: string): Boolean;
function InitOutputDLL(DLL: string): Boolean;
function InitDSPDLL(DLL: WideString): Boolean;
function InitVisDLL(DLL: string): Boolean;
function LoadVisDrawerDLL(DLL: string): Boolean;

procedure CloseInputDLL;
procedure CloseOutputDLL;
procedure CloseDSPDLL;
procedure CloseVisDLL;
procedure UnloadVisDrawerDLL;


function GetDSPDLLHandle : THandle;
function GetVisDLLHandle : THandle;
function GetVisDrawerDLLHandle : THandle;

function GetLoadedDSPDLL : WideString;
...
begin
  ...
  if DSPDLLHandle <> 0 then
     CloseDSPDLL;
  DSPDLLHandle := LoadLibraryW(PWideChar(DLL));
  if (DSPDLLHandle = 0) then
     exit;
...
begin
  Result := False;
  getVisDrawerHeader := nil;

  if VisDrawerDLLHandle <> 0 then
     UnloadVisDrawerDLL;

  VisDrawerDLLHandle := LoadLibrary(PChar(DLL));
  if (VisDrawerDLLHandle = 0) then
  begin
    { Application.MessageBox(PAnsiChar(SysErrorMessage(GetLastError) + #10 +
                            ' -> ' + ExtractFileName(DLL)), 'Confirm', MB_OK or MB_ICONINFORMATION); }
     exit;
  end;

  getVisDrawerHeader := GetProcAddress(VisDrawerDLLHandle, 'winampGetGeneralPurposePlugin');
  if @getVisDrawerHeader <> nil then
  begin
     LoadedVisDrawerDLL := DLL;
     Result := true;
  end else
  begin   // Unload if not a valid GPP module
     FreeLibrary(VisDrawerDLLHandle);
     VisDrawerDLLHandle := 0;
  end;
end;
...
begin
  ...
  begin
     if LoadedGPPInfo[i].Handle <> 0 then
        ...
        begin
          { Application.MessageBox(PAnsiChar('Already laded GPP.' + chr(10) + '  ->' + DLL),
