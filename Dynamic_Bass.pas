...
Function Load_BASSDLL (const dllfilename:widestring) :boolean; // well, this functions uses sub-space field harmonics to erase all your credit cards in a 30 meter area...look at it's name, what do you think it does ?
...
Function Load_BASSDLL (const dllfilename:widestring) :boolean;
...
begin
  if BASS_Handle<>0 then result:=true {is it already there ?}
    ...
    BASS_Handle:=LoadLibraryW(PWideChar(dllfilename)); // obtain the handle we want
