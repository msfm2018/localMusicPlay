...
uses
{$IFDEF VER230}
  Winapi.Windows, System.Classes, Winapi.GDIPAPI, Winapi.GDIPOBJ, Winapi.GDIPUTIL,
  System.SysUtils, Winapi.ActiveX;
{$ELSE}
  Windows, Classes, GDIPAPI, GDIPOBJ,Winapi.ActiveX;
{$ENDIF}
...
//    FLyricBackImage: TGPImage;
    ...
    procedure DrawStrToImage(var DestBitmap: TGPBitmap; AStr, FontName: string; AWidth: Integer;
  ...
  public
    ...
    procedure SetProgress(AProgress: Single);
    procedure SetWidthAndHeight(AWidth, AHeight: Integer);
    function  GetTextWidth(Str: String; Alignment: TStringAlignment = StringAlignmentNear):Integer;

    property ShowBack: Boolean read FShowLyricBack write SetShowBackImage;
    property ShowFlags: TGDIShowFlags read FShowFlags write SetSingleOrDoubleLine;
    property FirstString: string read FFirstStr write FFirstStr;
    property NextString : string read FNextStr  write FNextStr;
...
begin
  ...
  FShowLyricBack := False;
//
//  FResStream := TResourceStream.Create(HInstance, 'LRCBACK', 'PNGIMAGE');
//  IStreamPng := TStreamAdapter.Create(FResStream, soOwned);
//  FLyricBackImage := TGPImage.Create(IStreamPng);
...
procedure TGDIDrawLyric.DrawStrToImage(var DestBitmap: TGPBitmap; AStr,
       FontName: string; AWidth: Integer;
       StartColor, EndColor: TGPColor; FontHeight: Integer; ForeColor: Boolean;
...
function  TGDIDrawLyric.GetTextWidth(Str: String; Alignment: TStringAlignment):Integer;
var
  Path: TGPGraphicsPath;
  FontFamily:TGPFontFamily ;
  strFormat:TGPStringFormat;
...
procedure TGDIDrawLyric.Invalidate;
begin
  DrawLyricBitmapFirst;
end;
...
procedure TGDIDrawLyric.SetProgress(AProgress: Single);
begin
  FProgress := AProgress;
  UpdateDisplay;
end;

procedure TGDIDrawLyric.SetShowBackImage(Value: Boolean);
begin
  if FShowLyricBack <> Value then
  begin
    FShowLyricBack := Value;
    UpdateDisplay;
  end;
end;
...
var
  gBack, gBack2, gFore, gLyricBack:TGPGraphics;
  Pen: TGPPen;
  Winsize:TSize;
  SrcPoint:TPoint;
...
begin
    ...
    // 画背景
//    if FShowLyricBack then
//    begin
//      gLyricBack := TGPGraphics.Create(MemHDC);
//      Pen := TGPPen.Create(MakeColor(65, 1, 3, 3), 1);
//      gLyricBack.DrawImage(FLyricBackImage, MakeRect(0, 0, FWidth, FHeight), 0, 0,
//             FLyricBackImage.GetWidth - 1, FLyricBackImage.GetHeight, UnitPixel);
//      gLyricBack.DrawRectangle(Pen, MakeRect(0, 0, FWidth - 1, FHeight - 1));
//      Pen.Free;
//      gLyricBack.Free;
//    end;
      ...
      sfSingle :
        begin
          ...
          begin
            ...
            gFore.SetClip(MakeRect(TmpLeft2, TmpTop2, FStrWidth1 , TmpHeight * FProgress)) ;
        ...
        begin
          ...
          begin
            ...
            case FDrawMod of
               0 :
                 begin
                   gFore.SetClip(MakeRect(0.0, 10, FProgress, FFontHeight)) ;
